...
int oledSetCursor(int row, int column)
{
	...
	if (row < 0 || row >= OLED_EXP_CHAR_ROWS) {
		printf( "ERROR: Attempting to set cursor to invalid row '%d'\n", row);
		return EXIT_FAILURE;
	}
...
int oledWriteChar(char c)
{
	...
	if (charIndex >= 0 && charIndex < sizeof(asciiTable) / sizeof(asciiTable[0])) {
	    ...
	    if (_cursorInRow == OLED_EXP_CHAR_COLUMNS) { 												// ТУТ БЫЛО - 1
	    	_cursorInRow 	= 0;
	    }
	    else {
	    	_cursorInRow++;
...
// 		for(int j = 0 ;  j < 21 ; j++){
//
// 			int s = oledSetCursorByPixel(i,j*6);
// 			oledWriteChar(a);
// 			a++;
// 			if(i == 5){
// 				//printf("j = %i symbol =  %c status = %i\n",j , a, s);
//
// 			}
// 		}
// 		printf("%c\n", a);
